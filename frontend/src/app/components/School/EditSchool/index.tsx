...
export default function EditSchoolForm({
  ...
  const [products, setProduct] = useState({...product, description: 'lorem eption description here'});

  const [isOpen, setIsOpen] = useState(true);
  const [selectedEmoji, setSelectedEmoji] = useState<string | null>(null);
  const [isEmojiPickerOpen, setIsEmojiPickerOpen] = useState(false);
  const [links, setLinks] = useState<{ text: string; url: string }[]>([]);
  const [bannerPreview, setBannerPreview] = useState(product.image);
  const [thumbnailPreview, setThumbnailPreview] = useState(product.image);
  const [schoolName, setSchoolName] = useState<string>(product.title);
      ...
      <>
        <Drawer open={isOpen} onOpenChange={onClose}>
          <DrawerContent className="h-full  !max-h-[100vh]">
            <DrawerTitle className="hidden">Edit School</DrawerTitle>
            ...
            <form id="schoolForm" onSubmit={handleSubmit}>
              <div className="mb-4">
                <div className="flex items-center mb-2">
                  ...
                  {/* Character Counter */}
                  <span className="text-xs text-gray-500 ml-65">
