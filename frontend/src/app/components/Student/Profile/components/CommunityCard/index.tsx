...
import { CommunitySvg } from '@/app/components/svg';
...
export const CommunityCard = ({course}: CommunityCardProps) => {
			...
			<div className='p-4'>
				...
				<div className='flex gap-3 items-center my-4'>
					<div>
						<Image
							alt={course.title}
							className=' w-[50px] h-[50px] shadow-lg rounded-lg object-cover'
					...
					<div className=''>
						...
						<p className='text-xs text-gray-600 '>Cofounder of DevStack</p>
					</div>
				</div>
				...
				<div className='flex rounded-md overflow-hidden bg-[#13C4CC] text-white  shadow-md mb-2'>
					<button className='p-2 flex-grow text-center text-sm rounded-none  hover:bg-teal-500 transition-colors'>
						Join Now
					</button>
				</div>
				{isOpen && isMobile && (
			...
			<div className='flex items-center justify-between text-sm bg-[rgb(248,_249,_251)] p-4 border-t rounded-b-lg'>
				...
				<div className='relative group flex items-center gap-1'>
					<div className='text-[#4287C4] w-4 h-4'>
						{<CommunitySvg className='size-4 fill-black'/>}
					</div>
					<span className='text-[14px] font-semibold text-[rgb(0, 0, 0)]'>
						{course.postCount}k
					</span>
