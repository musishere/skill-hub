...
import { getExploreSessions } from "@/lib/api-client";
...
export const TrendingSessions = () => {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        setLoading(true);
        console.log('🔄 TrendingSessions: Starting API call to /api/client/explore/sessions');
        const data = await getExploreSessions();
        console.log('✅ TrendingSessions: API Response received:', data);
        console.log('📊 TrendingSessions: Data length:', data?.length || 0);
        ...
        const transformedSessions = data.map((session, index) => {
          ...
          return {
            title: session.title,
            img: session.image || fallbackImages[index % fallbackImages.length],
            instructorimg: session.image || fallbackImages[index % fallbackImages.length],
            instructor: session.author || "Session Instructor",
            content: session.points?.[0]?.text || "Learn from industry experts...",
            rating: session.rating?.toString() || "4.5",
            reviews: session.reviews?.toString() || "1k",
            students: session.students || "10k",
            duration: "Group",
            icon: {
              studentSvg: <UserSVG />,
              starSvg: <StarSvg className="w-4 h-4 text-yellow-500" />,
        ...
        console.log('🔄 TrendingSessions: Transformed data:', transformedSessions);
        setSessions(transformedSessions);
      } catch (err) {
        console.error('❌ TrendingSessions: API Error:', err);
        setError(err instanceof Error ? err.message : 'Failed to fetch sessions');
        console.log('🔄 TrendingSessions: Using fallback data due to error');
        ...
        <div className="max-w-7xl mx-auto w-full">
          <div className="flex items-center gap-4 mb-6">
            <div className="w-11 h-11 bg-gray-100 rounded-lg flex items-center justify-center">
              <SessionSvg className="w-6 h-6" fill="#1c4ed8" />
            </div>
  ...
  const currentSessions = error ? fallbackSessions : sessions;
  ...
  return (
    <div className="mb-4 bg-white px-5 py-6 rounded-2xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {currentSessions.map((course, index) => (
          <div
            key={index}
