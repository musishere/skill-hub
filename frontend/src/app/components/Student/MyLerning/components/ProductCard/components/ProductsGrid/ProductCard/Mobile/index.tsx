...
const handleAction = (action: string, type: string, title: string) => {
  console.log('Action clicked:', action, type, title);

  switch (action) {
    case 'View Outline':
      // Navigate to course outline
      window.open(`/course/${title.toLowerCase().replace(/\s+/g, '-')}/outline`, '_blank');
      break;
    case 'View RSVP':
      // Navigate to session RSVP
      window.open(`/session/${title.toLowerCase().replace(/\s+/g, '-')}/rsvp`, '_blank');
      break;
    case 'View Sales History':
      // Navigate to sales history
      window.open(`/product/${title.toLowerCase().replace(/\s+/g, '-')}/sales`, '_blank');
      break;
    case 'Continue Learning':
      // Navigate to course content
      window.open(`/course/${title.toLowerCase().replace(/\s+/g, '-')}/learn`, '_blank');
      break;
    default:
      ...
      <div className="flex flex-col">
        ...
        <div className="flex flex-col p-4">
          ...
          <div className="relative flex items-center mt-2 w-full">
            <button
              onClick={() => handleAction(action || '', type, title)}
              className="w-full h-[40px] py-2 px-0 bg-[#00BCD4] text-white border-none text-sm rounded-md font-semibold cursor-pointer text-center transition-all duration-200 ease-in-out hover:bg-[#00ACC1] active:bg-[#0fa3aa] active:transform active:translate-y-[1px] focus:outline-none focus:shadow-custom"
            >
              {action}
            </button>
          </div>
